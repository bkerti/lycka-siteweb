...
export const createHomeModel = async (homeModelData: Omit<HomeModel, 'id'>): Promise<HomeModel> => {
  ...
  const { rows } = await sql<HomeModel>`
    ...
    VALUES (${name}, ${price}, ${sqm}, ${description}, ${JSON.stringify(media)}::jsonb, ${category}, ${floors}, ${livingRooms}) 
    RETURNING *
  `;
  return rows[0];
};
...
export const updateHomeModel = async (id: string, homeModelData: Partial<Omit<HomeModel, 'id'>>): Promise<HomeModel | null> => {
  ...
  const { rows } = await sql<HomeModel>`
    UPDATE home_models 
    SET name = ${name}, price = ${price}, sqm = ${sqm}, description = ${description}, media = ${JSON.stringify(media)}::jsonb, category = ${category}, floors = ${floors}, livingRooms = ${livingRooms} 
    WHERE id = ${id} 
    RETURNING *
  `;
  return rows[0] || null;
