...
export const createProject = async (projectData: Omit<Project, 'id'>): Promise<Project> => {
  ...
  const { rows } = await sql<Project>`
    ...
    VALUES (${title}, ${category}, ${description}, ${JSON.stringify(media)}, ${status}, ${location}, ${year}, ${price}) 
    RETURNING *
  `;
  return rows[0];
};
...
export const updateProject = async (id: string, projectData: Partial<Omit<Project, 'id'>>): Promise<Project | null> => {
  ...
  const { rows } = await sql<Project>`
    UPDATE projects 
    SET title = ${title}, category = ${category}, description = ${description}, media = ${JSON.stringify(media)}, status = ${status}, location = ${location}, year = ${year}, price = ${price} 
    WHERE id = ${id} 
    RETURNING *
  `;
  return rows[0] || null;
