    ...
    try {
      ...
      for (const project of projectsToSeed) {
        await client.sql`
          ...
          VALUES (${project.title}, ${project.category}, ${project.description}, ${JSON.stringify(project.media)}, ${project.status}, ${project.location}, ${project.year}, ${project.price})
      ...
      for (const service of servicesToSeed) {
        await client.sql`
          ...
          VALUES (${service.title}, ${service.description}, ${service.icon}, ${service.imageUrl}, ${JSON.stringify(service.features)})
          ON CONFLICT (title) DO NOTHING;
        `;
      }
      ...
      for (const model of homeModelsToSeed) {
        await client.sql`
          ...
          VALUES (${model.name}, ${model.price}, ${model.sqm}, ${model.description}, ${JSON.stringify(model.media)}, ${model.category}, ${model.floors}, ${model.livingRooms})
      ...
      for (const article of blogToSeed) {
        await client.sql`
          ...
          VALUES (${article.title}, ${article.content}, ${JSON.stringify(article.media)})
          ON CONFLICT (title) DO NOTHING;
        `;
      }
