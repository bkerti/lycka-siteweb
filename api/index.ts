    ...
    try {
      ...
      for (const project of projectsToSeed) {
        await client.sql`
          ...
          VALUES (${project.title}, ${project.category}, ${project.description}, ${project.media}, ${project.status}, ${project.location}, ${project.year}, ${project.price})
      ...
      for (const service of servicesToSeed) {
        await client.sql`
          ...
          VALUES (${service.title}, ${service.description}, ${service.icon}, ${service.imageUrl}, ${service.features})
          ON CONFLICT (title) DO NOTHING;
        `;
      }
      ...
      for (const model of homeModelsToSeed) {
        await client.sql`
          ...
          VALUES (${model.name}, ${model.price}, ${model.sqm}, ${model.description}, ${model.media}, ${model.category}, ${model.floors}, ${model.livingRooms})
      ...
      for (const article of blogToSeed) {
        await client.sql`
          ...
          VALUES (${article.title}, ${article.content}, ${article.media})
          ON CONFLICT (title) DO NOTHING;
        `;
      }
